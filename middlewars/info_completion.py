...
class InfoCompletion:
    ...
    def save_lyric(self,lrc_name:str,netease_id:str) -> str:
        ...
        res = self.get_request(lyric_url).json()
    ...
    def get_request(self,url:str):
        ...
        res = get(url=url,headers=headers,verify=False)
        if res.status_code != 200:
            raise RuntimeError("请求错误")
        return res
    ...
    def _completion_math_artist(self):
        ...
        for i in sql_res:
            ...
            try:
                ...
                response = self.get_request(url=url).json()
                ...
                logger.error(format_exc())
    # 匹配歌词
    def _completion_match_lyrics_netease_id(self):
        def match_mid_lyrics(res:dict,info:dict):
            mid = info.get("mid")
            title = info.get("title")
            artist = info.get("artist")
            album = info.get("album")
            ...
            if 'songs' in result:
                ...
                for i in songs:
                    name = i.get("name")
                    m_netease_id = i.get("id")
                    al = i.get("al")
                    album_name = al.get("name")
                    for ar in i.get("ar"):
                        artist_name = ar.get("name")
                        if fuzz.ratio(artist_name,artist) > 20 and fuzz.ratio(name,title) > 20 and fuzz.ratio(album,album_name) > 20:
                            match_res = True
                            break
                    if match_res:
                        break
                    ...
                    return match_res
                lyric_path = self.save_lyric(mid,m_netease_id)
                update_media_sql = f"""update media_file set netease_id="{m_netease_id}",lyrics="{lyric_path}" where id="{mid}"; """
                sql_con.sql2commit(update_media_sql)
                return match_res
            return match_res

        sql_con = Sqlite_con()
        get_lack_lyrics_sql = "select id,netease_id,title,album,artist from media_file where lyrics='None' and album <> 'none' and artist <> 'unkown';"
        sql_res = [{"mid":i[0],"netease_id":i[1],"title":i[2],"album":i[3],"artist":i[4]} for i in sql_con.sql2commit(get_lack_lyrics_sql)]
        for i in sql_res:
            if i.get("netease_id"):
                logger.info(f"{i}  歌词下载")
                self.save_lyric(lrc_name=i.get("mid"),netease_id=i.get("netease_id"))
            else:
                title,artist,album = i.get("title"),i.get("artist"),i.get("album")
                url = self.base_url + f'/cloudsearch?keywords={title} {artist} {album}'
                while True:
                    try:
                        res = self.get_request(url=url).json()
                        break
                    except:
                        logger.error(format_exc())
                        time.sleep(3)
                        continue
                match_res = match_mid_lyrics(res=res,info=i)
                if match_res:
                    logger.success(f"{i}  匹配成功")
                else:
                    logger.warning(f"{i}  匹配失败")


    # 音乐信息刮削，目前支持网易云音乐
    def _completion(self):
        curdate = datetime.now().strftime("%Y-%m-%d")
        logger.add(path.join(getcwd(),"log",f"info_completion_{curdate}.log"),colorize=True)
        try:
            self._completion_artist()
            self._completion_album()
            self._completion_math_artist()
            self._completion_match_lyrics_netease_id()
            # 计算数据
