...
class InfoCompletion:
    ...
    # 异步请求
    async def get_info(self,info:dict,url:str,sem:asyncio.Semaphore):
        async with sem:
    ...
    def search_song_info(self,info_list:list):
        ...
        while info_list:
            ...
            for i in info_list_cp[:50]:
                ...
                tasks.append(asyncio.ensure_future(self.get_info(info=i,url=url,sem=sem)))
    ...
    def _completion(self):
        ...
        logger.add(getcwd(),"log",f"info_completion_{curdate}.log")
        logger.info("信息收集开始")
        try:
            yaml_conf = YamlConfig()
            using_db = yaml_conf.check_sys_usingdb()
    ...
    def regular_start_completion(self):
        def start_fun():
            while True:
                curtime = datetime.now("%H:%M")
                if curtime == "01:30":
                    self._completion()
                time.sleep(60)
        t = Thread(target=start_fun)
        t.setDaemon(True)
        t.start()
    # 手动启动刮削功能
    def start_completion(self):
        try:
            t = Thread(target=self._completion)
            t.setDaemon(True)
            t.start()
