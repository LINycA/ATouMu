...
class Playlist:
    ...
    def playlist_add(self,name:str,comment:str,public:bool,user_id:str) -> Response:
        ...
        pid = uuid()
        playlist_insert_sql = f"""insert into playlist(id,name,comment,public,created_at,updated_at,owner_id) values("{pid}","{name}","{comment}",{public},"{curdate}","{curdate}","{user_id}");"""
